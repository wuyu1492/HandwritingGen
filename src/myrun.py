...
def main():
    ...
    parser.add_argument('--log_dir', type=str, default='./mylogs/', help='location, relative to execution, of log files')
    parser.add_argument('--data_dir', type=str, default='../data', help='location, relative to execution, of data')
    parser.add_argument('--save_path', type=str, default='mysaved/model.ckpt', help='location to save model')
...
def train_model(args):
    ...
    for e in range(int(global_step/args.nbatches), args.nepochs):
        ...
        for b in range(global_step%args.nbatches, args.nbatches):
            ...
            [train_loss,x,y,eos, _] = model.sess.run([model.cost, model.x, model.y, model.eos,  model.train_op], feed)
            ...
            if i % 10 is 0: logger.write("{}/{}, loss = {:.8f}, regloss = {:.5f}, valid_loss = {:.8f}, time = {:.3f}" \
